...
const TYPE_SUMMARY: Record<string, string> = {
  ...
  shader: 'Effect 着色器定义，供材质引用。当前预览仍使用内置粒子着色逻辑，导出时写入 Cocos Effect 引用。',
  mesh: '粒子网格模型。渲染模式为「网格 (Mesh)」时使用；当前预览仍以 Billboard 为主，网格渲染在后续版本完善。'
};

const TYPE_USAGE: Record<string, string> = {
  texture: '拖到 Inspector → 主贴图槽，或双击快速应用到当前发射器。',
  spriteFrame: '拖到主贴图槽（与贴图等效，导出语义更贴近 Cocos）。',
  material: '拖到 Inspector → 材质槽。additive 更亮更「发光」，alpha 更接近普通半透明。',
