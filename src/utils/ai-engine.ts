...
\`\`\`typescript
{
  ...
  "description": "简短描述",
  "mainModule": {
    "duration": 5,           // 运行时长（秒），0-60
    "capacity": 100,         // 最大粒子数，1-10000
    "loop": true,            // 是否循环
    "startLifetime": { "mode": "constant", "constant": 2 },  // 生命周期
    "startSpeed": { "mode": "constant", "constant": 5 },     // 初始速度
    "startSize3D": { "x": { "mode": "constant", "constant": 1 }, ... },
    "startColor": { "keys": [{ "time": 0, "color": [R,G,B,A] }, ...] },
    "gravityModifier": 0,    // 重力系数，-10到10
    "rateOverTime": 20,      // 每秒发射数
    "bursts": []             // 爆发发射 [{time, count}]
  },
  "shapeModule": {
    "enabled": true,
    "shapeType": "cone",     // box/sphere/hemisphere/cone/circle
    "radius": 1,
    "angle": 25              // 锥形角度，0-90
  },
  "colorOverLifetime": { "enabled": true, "color": {...} },
  "sizeOverLifetime": { "enabled": false, "size": {...} },
...
## 规则
1. 只返回 JSON，不要任何额外文本
2. 根据用户描述选择合适的 shapeType
3. 火焰用 cone，雪花用 box，爆炸用 sphere，魔法用 sphere
4. 颜色使用 RGBA 格式，每个通道 0-1
5. 合理设置粒子数量，不要过多

## 语义微调
如果用户要求微调（如"更快""更亮""更多"），只修改相关参数：
- 更快 → startSpeed × 1.5
- 更慢 → startSpeed × 0.6
- 更多 → rateOverTime × 1.8
- 更大 → startSize × 1.5
