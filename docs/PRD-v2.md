...
> **FX Studio 是一款面向游戏引擎（Cocos Creator 3.8）的通用粒子特效编辑器，提供层级化特效组合、资产引用管理与实时预览；AI 作为可选辅助能力，用于快速生成初稿与语义微调。**

### 1.3 设计原则

1. **编辑器优先**：布局、快捷键、工作流对标 Unity Particle System / Cocos Creator 粒子编辑器
2. **组合而非扁平**：项目 → 特效组 → 子特效（Emitter）→ 模块
3. **引用可追溯**：所有外部资源以 AssetRef 形式存在 IR 中，Inspector 可点选替换
4. **AI 可关闭**：无 API Key 时仍是完整可用的手动编辑器
5. **导出等价**：组合特效导出为 Cocos 多节点 Prefab，资产 UUID/路径完整保留
