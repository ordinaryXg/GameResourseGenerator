...
| 文档版本 | **v2.0** |
| 最后更新 | 2026-07-23 |
| 当前整体进度 | **~75–80%**（v2.0 正式版冲刺中） |
| 预估剩余工期 | **2–3 周**（Phase 6 收尾 + 发布门禁） |
...
Phase 6   收尾 & 发布（1 周）   🔄 ~30%   AI 适配 + E2E + 性能 + 文档
────────────────────────────────────────────────────────────────────
...
| 1.3 | 拖拽 reparent（`@dnd-kit` 或原生 DnD） | `HierarchyPanel` + store | P0 | 🔄 仅右键 reparent，缺拖拽 |
...
| 2.9 | 状态栏：项目名 + 选中节点 + 引擎目标 | `App.tsx` statusbar | P2 | 🔄 部分完成 |
...
| 3.5.7 | 贴图导入落盘 `{project}/assets/textures/` | `useAssetImport.ts`, Electron IPC | P2 | ⏳ 当前为 dataUrl 注册 |
...
| 步骤 | 任务 | 产出 | 优先级 | 状态 |
|------|------|------|--------|------|
| 6.1 | AI 生成目标改为「当前选中 Emitter」 | `ai-engine.ts`, `ChatPanel` | P1 | ⏳ 仍作用于 currentEffect |
...
### Phase 6 优先执行顺序（剩余 2–3 周）

```
Week 1  6.1 → 6.2 → 1.3（拖拽 reparent）→ 3.5.7（贴图落盘）
Week 2  4.7 → 2.9 → 6.5 → 6.6 → 6.7
...
| PR-3 | feat: hierarchy DnD and multi-select | 1.3, 1.10 | 🔄 |
