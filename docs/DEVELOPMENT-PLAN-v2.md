...
| **v2.0 正式版** | **2026-10** | 迁移 + 文档 + 冒烟 + 发布门禁 | ✅ 已交付 |
...
Phase 6   收尾 & 发布（1 周）   ✅ ~100%  AI 适配 + E2E + 性能 + 文档
────────────────────────────────────────────────────────────────────
...
| 4.7 | ExportModal 展示导出资产清单 | `ExportModal.tsx`, `buildProjectExportManifest` | P1 | ✅ |
...
| 5.8 | 模板库入口并入资产浏览器 / 预设 | `EmitterTemplatesModal` | P1 | ✅ |
...
| 6.3 | 模板库入口迁入资产浏览器 | `EmitterTemplatesModal` | P1 | ✅ |
| 6.4 | 更新 README / PRD / 用户指南 | `docs/`, `README.md` | P0 | ✅ |
| 6.5 | E2E 冒烟清单 | `docs/E2E-SMOKE.md` | P0 | ✅ |
| 6.6 | 性能 profiling：5×200 粒子 | `preview-performance.test.ts` + E2E P1 | P1 | ✅ |
| 6.7 | 发布门禁逐项验收 | `docs/RELEASE-GATE.md` | P0 | ✅ |
| 6.8 | Electron 生产构建 + 安装包 smoke | `npm run electron:build` | P0 | ✅ |
| 6.9 | CI：`build` + `vitest` 自动化 | `.github/workflows/ci.yml` | P1 | ✅ |

### Phase 6 优先执行顺序（剩余 1–2 周）

```
Week 1  ✅ 6.1 → 6.2 → 1.3 → 3.5.7
Week 2  ✅ 4.7 → 6.5 → 6.6 → 6.7 → 6.9
Week 3  ✅ 6.4 → 6.8 → 门禁签核 → v2.0 tag
```
...
| PR-10 | feat: multi-node prefab import | 4.5–4.7 | 🔄 4.7 ✅ |
| PR-11 | feat: v1 migration and preset projects | 5.4–5.6 | ✅ |
| PR-12 | chore: v2.0 release — E2E, gate, CI | 6.5–6.7, 6.9 | 🔄 |
